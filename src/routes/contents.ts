...
- ⚠️ **아래 단어를 제목에 절대 사용 금지** (Google이 scaled content로 판정):
  ...
  → 대신 구체적이고 독특한 표현 사용
- ⚠️ **커뮤니티/포럼명 절대 사용 금지** — 제목, 본문, slug 어디에도 넣지 마세요:
  "더쿠", "디시", "디씨", "에펨코리아", "에펨", "뽐뿌", "클리앙", "루리웹", "인벤", "오유", "일베", "여시", "네이트판", "인스티즈", "82cook", "쿨앤조이", "보배드림", "블라인드"
  → 키워드에 커뮤니티명이 포함되어 있더라도, 해당 커뮤니티명은 제거하고 핵심 의료 키워드만으로 글을 작성하세요
...
## ⛔ 실명(본명) 사용 절대 금지 — 개인정보보호법 위반
⚠️ 이 규칙은 법적 의무입니다. 어기면 글 전체가 무효됩니다.
- **환자 사례, 후기, 경험담에 실제 이름(성+이름)을 절대 사용하지 마세요**
- 김미영, 박영수, 이지은, 최수진, 정민호 등 실제 이름 형태 전부 금지
- ✅ 허용: "김모 씨", "50대 여성 환자분", "A씨", "한 환자분", "최근 내원하신 분"
- ✅ 허용: 성만 사용 ("김 씨", "이 씨", "박 씨")
- ❌ 금지: "김미영 씨", "박영수 환자", "이지은 님" (성+이름 조합 전부 금지)
- 간호사, 직원 등 스태프 이름도 사용 금지
- 단, 저자(원장) 이름은 사용 가능 (예: "문석준 원장"은 OK)
- 모든 환자 사례는 익명화: "~모 씨", "~대 환자분", 직업+연령 조합만 사용
...
위 시스템 프롬프트의 모든 규칙을 엄격히 준수하고, 반드시 유효한 JSON만 출력하세요.`
  ...
  const response = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    signal: AbortSignal.timeout(240000) // 4분 타임아웃
  })

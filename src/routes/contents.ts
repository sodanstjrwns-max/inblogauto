...
## 출력 형식 (유효한 JSON만)
{
  ...
  "content_html": "완전한 HTML 본문 (${structure.label} 구조, H2 ${h2Count}개)",
...
2. **<h2> ${h2Count}개**: ${structure.label} 구조에 맞게 (FAQ H2 포함)
3. 각 H2 내: 공감 + 답변 + **조건부 수치** + 임상 경험 1문장+ (300~500자)
4. <h3> 적극 활용
5. <ul>/<ol> 체크리스트, 단계별 리스트
6. 비교 시 <table>
7. 본문 핵심 H2 2~3곳에 \`<!-- IMAGE_SLOT:설명 -->\` 삽입
8. FAQ: <h2>자주 묻는 질문</h2> 아래 <details><summary>Q</summary><p>A</p></details> 5~7개
9. 마무리 <p>: 핵심 요약 + 임파워먼트 (병원 예약 유도 금지)
10. 면책: <div style="background:#f0f7ff;padding:16px;border-radius:8px;margin-top:32px;font-size:13px;color:#555;border-left:3px solid #3b82f6"><strong>📋 의료 정보 안내</strong><br>${disclaimer}</div>
